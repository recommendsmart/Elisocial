...
class SettingsForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if ($frontend_library_default) {
      $this->messenger()->addWarning($this->t('When changing the settings, all configured "Column width classes" will be purged.'));
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue(Constants::FRONTEND_LIBRARY) == Constants::BOOTSTRAP) {
      ...
      // Display a message.
      $this->messenger()->addWarning(t('You have selected Bootstrap, to display the layout properly in the frontend: install the @link module & select version 4.x or implement Bootstrap (v4) in your theme.', ['@link' => $link]));
    }
    ...
    if (!$old_frontend_library) {
      ...
      // Display a message.
      $this->messenger()->addStatus(t('Settings have been saved, @link to add a Dynamic Layout!', array('@link' => $new_layout_link)));
    }
  ...
  public function updateValues(
    ...
    if ($old_frontend_library != $new_frontend_library) {
      $settings->purgeColumnWidthNumbers($last_column_number, $new_column_prefix);
      if ($old_frontend_library) {
        $this->messenger()->addError($this->t('All column widths have been purged, please reconfigure your layouts!'));
      }
