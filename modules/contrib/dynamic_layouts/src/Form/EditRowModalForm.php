...
class EditRowModalForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $options = NULL) {
    ...
    $dynamic_layout_id = \Drupal::request()->get(Constants::DYNAMIC_LAYOUT_ID);
    $row_id = \Drupal::request()->get(Constants::ROW_ID);

    $row_classes = '';
    /* @var \Drupal\dynamic_layouts\DynamicLayoutInterface $config_entity */
    if ($dynamic_layout_id !== NULL &&
      $config_entity = $this->entityTypeManager->getStorage('dynamic_layout')->load($dynamic_layout_id)) {
  ...
  public function submitModalFormAjax(array $form, FormStateInterface $form_state) {
    ...
    if ($form_state->hasAnyErrors()) {
      ...
      if ($row_classes = $form_state->getValue('custom_row_classes')) {
